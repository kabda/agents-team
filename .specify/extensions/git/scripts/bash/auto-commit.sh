...
_feature_dir=""
if [ "$EVENT_NAME" = "after_specify" ] && [ -f "$REPO_ROOT/.specify/feature.json" ]; then
    _feature_dir=$(sed -n 's/.*"feature_directory"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$REPO_ROOT/.specify/feature.json" | head -1)
fi

# Check if there are changes to commit. after_specify only considers the
# current feature spec artifacts so unrelated dirty files do not trigger a
# failing empty commit attempt.
if [ "$EVENT_NAME" = "after_specify" ] && [ -n "$_feature_dir" ]; then
    _has_changes=false
    if ! git diff --quiet HEAD -- .specify/feature.json "$_feature_dir" 2>/dev/null; then
        _has_changes=true
    fi
    if ! git diff --cached --quiet -- .specify/feature.json "$_feature_dir" 2>/dev/null; then
        _has_changes=true
    fi
    if [ -n "$(git ls-files --others --exclude-standard -- .specify/feature.json "$_feature_dir" 2>/dev/null)" ]; then
        _has_changes=true
    fi
...
elif git diff --quiet HEAD 2>/dev/null && git diff --cached --quiet 2>/dev/null && [ -z "$(git ls-files --others --exclude-standard 2>/dev/null)" ]; then
    echo "[specify] No changes to commit after $EVENT_NAME" >&2
    exit 0
fi
...
# Stage and commit. after_specify should only commit the current feature spec
# artifacts, otherwise a dirty worktree can pull unrelated files into the hook.
if [ "$EVENT_NAME" = "after_specify" ] && [ -f "$REPO_ROOT/.specify/feature.json" ]; then
    if [ -n "$_feature_dir" ]; then
        _git_out=$(git add -- .specify/feature.json "$_feature_dir" 2>&1) || { echo "[specify] Error: git add failed: $_git_out" >&2; exit 1; }
    else
